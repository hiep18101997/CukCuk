...
/**
 * - Mục đích Class :Adapter Recyvlerview màn hóa đơn - @created_by Hoàng Hiệp on 4/5/2019
 */
public class BillAdapter extends RecyclerView.Adapter<BillAdapter.ViewHolder> {
  ...
  public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
    try {
      ...
      holder.tvUnitPrice.setText(NumberFormat.getNumberInstance(Locale.US)
          .format(mOrders.get(position).getDish().getCost()));
      holder.tvAmount.setText(NumberFormat.getNumberInstance(Locale.US)
          .format(mOrders.get(position).getDish().getCost() * mOrders.get(position).getQuantity()));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
