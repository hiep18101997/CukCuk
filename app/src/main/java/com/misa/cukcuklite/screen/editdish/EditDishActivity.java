...
public class EditDishActivity extends AppCompatActivity implements IEditDishContract.IView,
  ...
  private void loadDataExist() {
    try {
      ...
      Glide.with(this).load(mPresenter.getBitmapFromAssets(this, currentDish.getIcon()))
          .into(ivProductIcon);
  ...
  public void showCalDialog() {
    try {
      FragmentManager fm = getSupportFragmentManager();
      CalculatorFragment calculatorFragment = CalculatorFragment
          .createInstance(tvCost.getText().toString(), new CalculatorFragment.IOnClickDone() {
            @Override
  ...
  private void showConfirmDialog() {
    ...
    ConfirmRemoveDialog inputDialog = new ConfirmRemoveDialog(
        getString(R.string.confirm_remove_dish), new ConfirmRemoveDialog.OnClickAccept() {
      @Override
