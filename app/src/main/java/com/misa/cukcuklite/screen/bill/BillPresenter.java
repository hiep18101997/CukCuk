...
/**
 * ‐ Presenter trong mô hình MVP cho màn hình Hóa đơn ‐ @created_by Hoàng Hiệp on 4/5/2019
 */
public class BillPresenter implements IBillContract.IPresenter {
    ...
    try {
      new AsyncTask<Void, Void, Void>() {
        ...
        protected Void doInBackground(Void... voids) {
          order.setPay(true);
          if (order.getId() == 0) {
            int orderId = (int) DatabaseClient.getInstance(mContext).getAppDatabase().mOrderDAO()
                .insertOrder(order);
            order.setId(orderId);
  ...
  /**
   * Mục đích method: Lấy ra Id hóa đơn
   *
   * @created_by Hoàng Hiệp on 4/6/2019
   */
  @SuppressLint("StaticFieldLeak")
    ...
    try {
      new AsyncTask<Void, Void, Integer>() {
        ...
        protected Integer doInBackground(Void... voids) {
          return DatabaseClient.getInstance(mContext).getAppDatabase().mBillDAO().getBillCount();
