    ...
    try {
      new AsyncTask<Void, Void, List<ReportDetail>>() {
        ...
        protected List<ReportDetail> doInBackground(Void... voids) {
          ...
          try {
            List<Bill> bills = DatabaseClient.getInstance(mContext).getAppDatabase().mBillDAO()
                .getBillBetweenDate(dates[0], dates[1]);
            reportDetails = new ArrayList<>();
            for (Bill bill : bills) {
              List<DishOrder> dishOrders = DatabaseClient.getInstance(mContext).getAppDatabase()
                  .mDishOrderDAO().getDishOrderByOrderId(bill.getOrderId());
              for (DishOrder dishOrder : dishOrders) {
                Dish dish = DatabaseClient.getInstance(mContext).getAppDatabase().mDishDAO()
                    .getDishById(dishOrder.getDishId());
                Unit unit = DatabaseClient.getInstance(mContext).getAppDatabase().mUnitDAO()
                    .getUnitById(dish.getUnitId());
                String name = dish.getName();
                String unitName = unit.getName();
                int quantity = dishOrder.getQuantity();
                long amount = dishOrder.getCost();
                ReportDetail reportDetail = new ReportDetail.Builder().setAmount(amount)
                    .setName(name).setQuantity(quantity).setUnit(unitName).build();
                if (quantity != 0) {
