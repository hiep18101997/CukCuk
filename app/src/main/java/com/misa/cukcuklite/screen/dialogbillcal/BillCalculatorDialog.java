...
/**
 * - Mục đích Class :Máy tính hóa đơn - @created_by Hoàng Hiệp on 4/12/2019
 */
public class BillCalculatorDialog extends DialogFragment implements
    BillCalculatorAdapter.OnClickSuggestMoney, View.OnClickListener {
  ...
  public BillCalculatorDialog(long amount, InputNumberFragment.DialogCallBack callBack) {
    mCallBack = callBack;
    textInput = String.valueOf(amount);
  }
  ...
  private void initView(View rootView) {
    try {
      ...
      if (textInput == null) {
        ...
        long amount = Long.valueOf(textInput);
        longs.add(amount);
        longs.add(amount + 1000);
        longs.add(amount - amount % 10 + 5000);
        longs.add(amount - amount % 10 + 10000);
      }
      showResult();
  ...
  public void onClick(long money) {
    mCallBack.setAmount(String.valueOf(money));
    dismiss();
  }
