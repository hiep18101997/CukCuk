...
/**
 * ‐ Presenter trong mô hình MVP cho màn hình Đặt món ‐ @created_by Hoàng Hiệp on 4/5/2019
 */
public class AddOrderPresenter implements IAddOrderContract.IPresenter {
        ...
        protected void onPostExecute(List<Dish> dishes) {
          ...
          for (Dish dish : dishes) {
            if (dish.isSell()) {
              ...
              DishOrder dishOrder = new DishOrder.Builder().setDishId(dish.getId()).setQuantity(0)
                  .setCost(dish.getCost()).build();
      ...
      if (isValidData(table, person, list)) {
        ...
        new AsyncTask<Void, Void, Long>() {
          ...
          protected Long doInBackground(Void... voids) {
            return DatabaseClient.getInstance(mContext).getAppDatabase().mOrderDAO()
                .insertOrder(mOrder);
          }
    ...
    new AsyncTask<Void, Void, Void>() {
      ...
      protected Void doInBackground(Void... voids) {
        for (DishOrder dishOrder : list) {
          ...
          DatabaseClient.getInstance(mContext).getAppDatabase().mDishOrderDAO()
              .insertDishOrder(dishOrder);
        }
        return null;
      }
  ...
  @SuppressLint("StaticFieldLeak")
  @Override
  public void editOrder(int idOrder, String table, String person,
