    ...
    try {
      new AsyncTask<Void, Boolean, Void>() {
        ...
        protected Void doInBackground(Void... voids) {
          ...
          Unit existUnit = DatabaseClient.getInstance(mContext).getAppDatabase().mUnitDAO()
              .getUnitByName(text);
          if (existUnit == null) {
            unitId[0] = (int) DatabaseClient.getInstance(mContext).getAppDatabase().mUnitDAO()
                .insertUnit(unit);
