...
/**
 * - Mục đích Class : Activity của màn hóa đơn - @created_by Hoàng Hiệp on 4/5/2019
 */
public class BillActivity extends AppCompatActivity implements IBillContract.IView,
  ...
  private void showBill() {
    try {
      tvTotalAmount
          .setText(NumberFormat.getNumberInstance(Locale.UK).format(getAmount(mOrder.getOrders())));
      tvCustomerAmount
          .setText(NumberFormat.getNumberInstance(Locale.UK).format(getAmount(mOrder.getOrders())));
      tvReturnAmount.setText(getString(R.string.key0));
    ...
    try {
      switch (v.getId()) {
        ...
        case R.id.btnDoneBelow:
          if ((long) NumberFormat.getNumberInstance(Locale.US)
              .parse(tvReturnAmount.getText().toString()) < 0) {
            Toast.makeText(this, getString(R.string.mess_bill), Toast.LENGTH_SHORT).show();
          } else {
            mPresenter.saveBill(mOrder, (long) NumberFormat.getNumberInstance(Locale.US)
                .parse(tvTotalAmount.getText().toString()));
          }
          break;
        case R.id.lnCustomerAmount:
          showDialogCal((long) NumberFormat.getNumberInstance(Locale.US)
              .parse(tvCustomerAmount.getText().toString()));
          break;
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private void showDialogCal(long amount) {
    try {
      BillCalculatorDialog billCalculatorDialog = new BillCalculatorDialog(amount,
          new InputNumberFragment.DialogCallBack() {
            @Override
            public void setAmount(String amount) {
              tvCustomerAmount.setText(NumberFormat.getNumberInstance(Locale.US)
                  .format(Long.parseLong(amount)));
              try {
                tvReturnAmount
                    .setText(NumberFormat.getNumberInstance(Locale.US)
                        .format(Long.parseLong(amount) - (long) NumberFormat
  ...
  @Override
  public void showBillId(Integer integer) {
    DecimalFormat format = new DecimalFormat("00000");
    tvRefNo.setText(format.format(integer));
  }
}
